...
for run_name, set_name in zip(run_names, set_names):
    ...
    envBAA_rsz = np.repeat(envBAA_rsz, 10, axis=1)
    
    envBAA_bool_rsz = np.where(envBAA_rsz>1,1,0).astype(np.uint8) # This trims all the BAA area that are active 1 time
    ...
    BAA_act_cumulative_path = os.path.join('/home/erri/Documents/PhD/Research/5_research_repos/PiQs_analysis/Maps/', run_name, run_name + '_envBAA_act_cumulative.tiff')
    # BAA_act_cumulative_path = os.path.join('/home/erri/Documents/PhD/Research/5_research_repos/PiQs_analysis/Maps/', run_name, run_name + '_envBAA_act_cumulative.txt')
    # BAA_act_cumulative = np.loadtxt(BAA_act_cumulative_path)
    
    BAA_act_cumulative_rsz = Image.open(BAA_act_cumulative_path).resize((126, 140), Image.LANCZOS)
    BAA_act_cumulative_rsz = np.array(BAA_act_cumulative_rsz)
    BAA_act_cumulative_rsz = np.repeat(BAA_act_cumulative_rsz, 10, axis=1) # Rescale the DEM (dx/dy = 10)
    ...
    BAA_act_cumulative_rsz_rsc = img_scaling_to_DEM(BAA_act_cumulative_rsz, scale, dx, dy, rot_angle) # envBAA resized, rescaled and rotated
    ...
    # # channel_background = plt.imshow(channel_plot, alpha=1.0)
...
#%%
    ...
    BAA_act_cumulative_rsz_rsc = BAA_act_cumulative_rsz_rsc[:,:envMAA_rsz.shape[1]]
    
    # With BAA cumulative map
    MAA_BAA_stack = np.stack((envMAA_rsz,BAA_act_cumulative_rsz_rsc))
    ...
    plt.scatter(MAA_BAA_dataset[0,:], MAA_BAA_dataset[1,:], s=2, c='red', marker='o')
