...
for jump in jumps:    
    ...
    bin_edges = [-50.0,-12.62,-5.69,-1.67,1.25,3.805,6.36,50.0]
    DEM_class_labels = [f"{bin_edges[i]},{bin_edges[i+1]}" for i in range(len(bin_edges) - 1)]
    ...
    for run in runs:
        ...
        computed_DEM_data = DEM_data[:-1,:,:] + DoD_data[:,:,:,0]
        ...
        for t in (range(0,DEM_data.shape[0]-1-jump)):
            ...
            # dist_array_dep_partial_class8_matrix = np.zeros((DEM_data.shape[0],num_bins))
            # dist_array_dep_partial_class9_matrix = np.zeros((DEM_data.shape[0],num_bins))
            # dist_array_dep_partial_class10_matrix = np.zeros((DEM_data.shape[0],num_bins))
            ...
            for x in range(DEM_data.shape[1]):
                for y in range(DEM_data.shape[2]):
                    ...
                    if bin_edges[0]<=matrix[t,x,y]<bin_edges[1]:
                        dist_array_sco_class1 = np.append(dist_array_sco_class1, DEM_data[t+jump,x,y])
                        dist_array_sco_partial_class1 = np.append(dist_array_sco_partial_class1, DEM_data[t+jump,x,y])
                    if bin_edges[1]<=matrix[t,x,y]<bin_edges[2]:
                        dist_array_sco_class2 = np.append(dist_array_sco_class2, DEM_data[t+jump,x,y])
                        dist_array_sco_partial_class2 = np.append(dist_array_sco_partial_class2, DEM_data[t+jump,x,y])
                    if bin_edges[2]<=matrix[t,x,y]<bin_edges[3]:
                        dist_array_sco_class3 = np.append(dist_array_sco_class3, DEM_data[t+jump,x,y])
                        dist_array_sco_partial_class3 = np.append(dist_array_sco_partial_class3, DEM_data[t+jump,x,y])
                    if bin_edges[3]<=matrix[t,x,y]<bin_edges[4]:
                        dist_array_sco_class4 = np.append(dist_array_sco_class4, DEM_data[t+jump,x,y])
                        dist_array_sco_partial_class4 = np.append(dist_array_sco_partial_class4, DEM_data[t+jump,x,y])
                    if bin_edges[4]<=matrix[t,x,y]<bin_edges[5]:
                        dist_array_sco_class5 = np.append(dist_array_sco_class5, DEM_data[t+jump,x,y])
                        dist_array_sco_partial_class5 = np.append(dist_array_sco_partial_class5, DEM_data[t+jump,x,y])
                    if bin_edges[5]<matrix[t,x,y]<bin_edges[6]:
                        dist_array_dep_class6 = np.append(dist_array_dep_class6, DEM_data[t+jump,x,y])
                        dist_array_dep_partial_class6 = np.append(dist_array_dep_partial_class6, DEM_data[t+jump,x,y])
                    if bin_edges[6]<=matrix[t,x,y]<bin_edges[7]:
                        dist_array_dep_class7 = np.append(dist_array_dep_class7, DEM_data[t+jump,x,y])
                        dist_array_dep_partial_class7 = np.append(dist_array_dep_partial_class7, DEM_data[t+jump,x,y])
    ...
    #%%
        ...
        np.savetxt(os.path.join(output_dir, 'DEM2DEM_partial_class1_distribution.txt'), np.round(dist_array_sco_partial_class1_matrix, decimals=2), delimiter=',')
        np.savetxt(os.path.join(output_dir, 'DEM2DEM_partial_class2_distribution.txt'), np.round(dist_array_sco_partial_class2_matrix, decimals=2), delimiter=',')
        np.savetxt(os.path.join(output_dir, 'DEM2DEM_partial_class3_distribution.txt'), np.round(dist_array_sco_partial_class3_matrix, decimals=2), delimiter=',')
        np.savetxt(os.path.join(output_dir, 'DEM2DEM_partial_class4_distribution.txt'), np.round(dist_array_sco_partial_class4_matrix, decimals=2), delimiter=',')
        np.savetxt(os.path.join(output_dir, 'DEM2DEM_partial_class5_distribution.txt'), np.round(dist_array_sco_partial_class5_matrix, decimals=2), delimiter=',')
        np.savetxt(os.path.join(output_dir, 'DEM2DEM_partial_class6_distribution.txt'), np.round(dist_array_dep_partial_class6_matrix, decimals=2), delimiter=',')
        np.savetxt(os.path.join(output_dir, 'DEM2DEM_partial_class7_distribution.txt'), np.round(dist_array_dep_partial_class7_matrix, decimals=2), delimiter=',')
