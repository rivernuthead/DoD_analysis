...
envMAW_arr_plot = np.where(np.isnan(np.abs(stack_bool[0,:,dim_x-analysis_window:, 0])), 0, np.abs(stack_bool[0,:,dim_x-analysis_window:, 0]))
...
envMAW_arr_plot = add_zeros_rows(envMAW_arr_plot, 0)
...
img2 = plt.imshow(envBAW, cmap='Reds', alpha=0.5, vmin=0, vmax=1)
