...
for run in RUNS:
    ...
    matrix_Wact_max=np.zeros((len(files)+3, len(files))) # Max active width report matrix
    matrix_Wact_min=np.zeros((len(files)+3, len(files))) # Minimum active width report matrix
    ...
    for h in range (0, len(files)-1):
        for k in range (0, len(files)-1-h):
            ...
            output_dir = os.path.join(home_dir, 'DoDs', 'DoD_'+run) # Set DoD outputs directory
            path_out = os.path.join(output_dir,  output_name) # Set DoD outputs path
            ...
            act_width_mean = (active_area/(DoD_vol.shape[1]*px_x))/(W*1000)*100 # Mean active width [%] - Wact/W
            act_width_array = np.array([np.nansum(active_pixel_count, axis=0)])*px_y # Array of the crosswise morphological active width [mm]
            ...
            if delta != 0:
                ...
                # Fill Wact/W matrix as below:
                # DoD step0  1-0   2-1   3-2   4-3   5-4   6-5   7-6   8-7   9-8  MIN MAX average
                # DoD step1  2-0   3-1   4-2   5-3   6-4   7-5   8-6   9-7        MIN MAX average
                # DoD step2  3-0   4-1   5-2   6-3   7-4   8-5   9-6              MIN MAX average
                # DoD step3  4-0   5-1   6-2   7-3   8-4   9-5                    MIN MAX average
                # DoD step4  5-0   6-1   7-2   8-3   9-4                          MIN MAX average
                # DoD step5  6-0   7-1   8-2   9-3                                MIN MAX average
                # DoD step6  7-0   8-1   9-2                                      MIN MAX average
                # DoD step7  8-0   9-1                                            MIN MAX average
                # DoD step8  9-0                                                  MIN MAX average
                
                matrix_Wact[delta-1,h]=act_width_mean
                matrix_Wact[delta-1,-2]=np.average(matrix_Wact[delta-1,:len(files)-delta])/1000/W
                matrix_Wact[delta-1,-1]=np.std(matrix_Wact[delta-1,:len(files)-delta])/1000/W
                
                # Fill MAX Wact matrix as below:
                matrix_Wact_max[delta-1,h]=np.max(act_width_array)
                matrix_Wact_max[delta-1,-1]=np.max(matrix_Wact_max[delta-1,:len(files)-delta])/1000/W
                
                # Fill MIN Wact matrix as below:
                matrix_Wact_min[delta-1,h]=np.min(act_width_array)
                matrix_Wact_min[delta-1,-1]=np.min(matrix_Wact_min[delta-1,:len(files)-delta])/1000/W
