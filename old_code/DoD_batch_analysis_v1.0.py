...
home_dir = '/home/erri/Documents/Research/morphological_approach/3_output_data/q1.0_2/2_prc_laser/'
...
sco_array=[]
active_area_array=[]
matrix_volumes=np.zeros((len(files)-1, len(files)-1))
matrix_dep=np.zeros((len(files)-1, len(files)-1))
matrix_sco=np.zeros((len(files)-1, len(files)-1))
...
#TODO check mask
array_mask_name, array_mask_path = 'array_mask.txt', '/home/erri/Documents/Research/morphological_approach/2_raw_data'
...
with open(home_dir +'report.txt', 'w') as fp:
    fp.write(header)
    fp.writelines(['\n'])
