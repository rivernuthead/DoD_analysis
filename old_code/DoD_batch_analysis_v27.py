...
for run in RUNS:
    ...
    for h in range (0, len(files)-1):
        for k in range (0, len(files)-1-h):
            ...
            # Perform the very first isolated_killer procedure
            DoD_filt_isol2, DoD_filt_isol2_gis = isolated_killer(DoD_filt_nature, thrs_zeros, 1, NaN)
            ...
            if save_plot_mode == 1 and DoD_plot_mode == 1:
            ...
            if DoD_plot_show_mode == 1 and DoD_plot_mode == 1:
                ...
                if not(os.path.exists(os.path.join(plot_out_dir,'filtmap'))):
                    os.mkdir(os.path.join(plot_out_dir,'filtmap'))
                if not(os.path.exists(os.path.join(plot_out_dir,'filtmap', run))):
                    os.mkdir(os.path.join(plot_out_dir,'filtmap', run))
                if DoD_plot_save_mode ==1:
                    plt.savefig(os.path.join(plot_out_dir,'filtmap',run,run +'_'+DoD_name[:-1]+'_filtmap.tif'), dpi=1000) # raster (png, jpg, rgb, tif), vector (pdf, eps), latex (pgf)
                else:
                    pass
                plt.show()
            else:
    ...
    if data_interpolation_mode == 1:
       ...
       # Build up volume temporal scale array for each runs
        ...
        if save_plot_mode == 1:
            ...
            plt.savefig(os.path.join(plot_dir, run +'_volume_interp.pdf'), dpi=200)
            plt.show()
            ...
            plt.savefig(os.path.join(plot_dir, run +'_morphW_interp.pdf'), dpi=200)
            plt.show()
    ...
    yData_Wact=matrix_Wact[:len(files)-1,0]
    yError_Wact=matrix_Wact[:len(files)-1,-1]
    ...
    if save_plot_mode==1:
        ...
        plt.savefig(os.path.join(plot_dir, run +'_dep_scour.pdf'), dpi=200)
        plt.show()
        ...
        axs.errorbar(xData1,yData_Wact, yError_Wact, linestyle='--', marker='^', color='purple')
        axs.set_ylim(bottom=0)
        axs.set_title(run + '- Active width')
        axs.set_xlabel('Exner time')
        axs.set_ylabel('Active width [%]')
        plt.savefig(os.path.join(plot_dir, run +'_active_width.pdf'), dpi=200)
        plt.show()
    else:
        pass
...
for i in range(0,len(RUNS)):
    data=np.loadtxt(os.path.join(out_dir, str(RUNS[i]), str(RUNS[i])+ '_morphWact_array.txt'), delimiter=',')
...
if run_mode==2:
    ...
    plt.savefig(os.path.join(plot_out_dir, 'morphWact_boxplot.pdf'), dpi=800)
    plt.show()
else:
    pass
