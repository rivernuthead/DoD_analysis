...
for run in RUNS:
    ...
    if plot_mean == 1:
        ...
        axs.set_ylim(bottom=0)
        # axs.set_xlim(bottom=0)
        plt.text(np.max(xData)*0.7, np.min(morphWact_mean_intCurve), 'Trun=' + str(dt) + 'min \n' + r'$\tau$=' + str(np.round(morphWact_mean_params_interp[2], decimals=1)) + 'min \n' + 'A = ' + str(np.round(morphWact_mean_params_interp[0], decimals=1)) + '\n' + 'Y= ' + str(np.round(morphWact_mean_params_interp[0]*(1-np.exp(-0/(morphWact_mean_params_interp[2]))), decimals=1)), fontdict=font)
