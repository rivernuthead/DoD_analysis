...
# -*- coding: utf-8 -*-
"""
Created on Fri Dec 15 13:45:38 2023
...
for x in range(0, dim_x):
    for y in range(0, dim_y):
            ...
            def prova(arr):
                ...
                if raw_slice_array[0] == 0:
                    ...
                    if np.all(raw_slice_array == 0):  # Check if the sliced array is full of zeros
                        ...
                        slice_array = raw_slice_array[n_zero:]
                ...
                if slice_array[0] != 0:
                    ...
                    time_array = np.append(time_array, (len(slice_array)-np.sum(np.abs(time_array)))*target_sign)
                ...
                return time_array, consecutive_minus_ones_array, consecutive_zeros_array, consecutive_minus_ones_array
            
            
            time_array, consecutive_ones_array, consecutive_zeros_array, consecutive_minus_ones_array = prova(raw_slice_array)
            
            distances, switch_counter = switch_distance(raw_slice_array)
        ...
        consecutive_ones_stack[:len(consecutive_ones_array), y, x] = consecutive_ones_array

        # FILL consecutive_zeros_stack
        consecutive_zeros_stack[:len(consecutive_zeros_array), y, x] = consecutive_zeros_array

        # FILL consecutive_minus_ones_stack
        consecutive_minus_ones_stack[:len(consecutive_minus_ones_array), y, x] = consecutive_minus_ones_array

        # FILL THE SWITCH MATRIX
        switch_matrix[y, x] = switch_counter
