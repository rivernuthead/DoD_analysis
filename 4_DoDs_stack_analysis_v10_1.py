...
envMAW = envMAW_arr
envBAW = Image.open('/home/erri/Documents/PhD/Research/5_research_repos/PiQs_analysis/Maps/'+run_name+'/Img0002_thrs8_cld.tiff')

envMAW_zoom = zoom(envMAW, (envBAW.size[1]/envMAW.shape[0], envBAW.size[0]/envMAW.shape[1])) 

# Convert matrices to images
img1 = plt.imshow(channel, alpha=1.0)
img3 = plt.imshow(np.where(envMAW_zoom==0, np.nan, envMAW_zoom), cmap='cool', aspect=0.1, origin='upper', alpha=0.5)
