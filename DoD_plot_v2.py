...
for run in RUNS:
    ...
    #            t=1   t=2   t=4   t=5   t=6   t=7   t=8   t=9   t=10
    ...
    comp_prc_array = [] # Array to collect the compensation process number of pixel fpr the DoDs
    thrs_prc_array = [] # Array to collect the threshold filter process number oof pixel for the
    active_envelope_area_array = [] 
    for i in range(0, stack.shape[0]-1):
        envelope = np.nansum(abs(stack_bool[:i+2,:,:,0]), axis=0) # Envelop of 1-timestep DoD
        envelope = np.where(envelope>0, 1, envelope) # Convert the envelop in a boolean map
        active_envelope_area = np.nansum(envelope) # Compute the active area
        act_DoD = abs(stack_bool[0,:,:,i+1]) # Extract the envelop of the DoD whithin which the 1-timestep DoD take place
        
        matrix = envelope-act_DoD # Compute the difference between the 1-timestep envelop and the boundary DoD
        
        comp_processes = np.nansum(matrix==+1) # Compute the number of compensation cell (+1)
        threshold_processes = np.nansum(matrix==-1) # Compute the number of threshold affected cell (-1)
        
        comp_prc_array = np.append(comp_prc_array, comp_processes) # Append data 
        thrs_prc_array = np.append(thrs_prc_array, threshold_processes) # Append data
        active_envelope_area_array = np.append(active_envelope_area_array, active_envelope_area) # Append data
    
    np.savetxt(os.path.join(report_dir, run, run + '_comp_analysis.txt'), comp_prc_array, delimiter='\t')
    np.savetxt(os.path.join(report_dir, run, run + '_thrs_analysis.txt'), thrs_prc_array, delimiter='\t')
    np.savetxt(os.path.join(report_dir, run, run + '_active_envel_area.txt'), active_envelope_area_array, delimiter='\t')
            

a = np.nansum(abs(stack_bool[:,:,:,0]), axis=0)
a=np.where(a>0, 1, a)
# a = np.nansum(a>0)
b = np.nansum(abs(stack_bool[:,:,:,1]), axis=0)
b=np.where(b>0, 1, b)
# b = np.nansum(b>0)
c=a-b
d = np.nansum(c>0)
