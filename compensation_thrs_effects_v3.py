...
for run in runs:
    ...
    np.save(os.path.join(report_dir, 'report_' + run, 'comp_thrs_analysis', run + '_envMAA_DoD_difference.npy'), np.round(report_stack, decimals=2))
    np.savetxt(os.path.join(report_dir, 'report_' + run, 'comp_thrs_analysis', run + '_compensation_dimless_width.txt'), np.round(report_comp_matrix, decimals=2), delimiter=',')
    np.savetxt(os.path.join(report_dir, 'report_' + run, 'comp_thrs_analysis', run + '_under_thrs_dimless_width.txt'), np.round(report_thrs_matrix, decimals=2), delimiter=',')
