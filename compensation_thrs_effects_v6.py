...
for run in runs:
    ...
    for i in range(0,stack.shape[3]-1): # For each delta
        ...
        for t in range(0,stack.shape[0]-1-index):
            ...
            # Compute the envelope keeping the the number of activation time
            ...
            for x in range(0,stack_bool.shape[2]):
                for y in range(0,stack_bool.shape[1]):
                    ...
                    compensation = np.any(envMA_array == 1) & (DoD_array == 0)
    ...
    np.savetxt(os.path.join(output_dir, run + '_compensation_dimless_width.txt'), report_compensation_pure_stack, delimiter=',', fmt = "%.4f")
    np.savetxt(os.path.join(output_dir, run + '_under_thrs_dimless_width.txt'), report_threshold_pure_stack, delimiter=',', fmt = "%.4f")
...
compensation_pure_test = compensation_pure_stack[1,:,:,0]
                    
    #         diff_matrix = envMAA - DoD
            
    #         # Define different situations
