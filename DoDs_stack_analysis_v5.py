...
# switch_matrix = np.where(np.isnan(act_time_stack),np.nan,1)*np.where(act_time_stack!=0,1,0)
# # So, this 3D matrix will be converted as a 2D metrix by a sum operation
# switch_matrix = np.sum(switch_matrix, axis=0)
...
if plot_mode ==1:
    for i in range(0,5):
        ...
        plt.title(run+' - Switch threshold set at: '+str(i))
