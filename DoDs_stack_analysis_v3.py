        ...
        if slice_array[0]!=0: # If the first entry of the sliced array is non-zero
            ...
            for i in range(0,len(slice_array)-1): # Loop over the sliced array
                ...
                if np.sign(a1)==np.sign(a2): # If two consecutive elements have the same naure
                    ...
                    time_array = np.append(time_array, count*target_sign) # The count stops when a switch occours or when one of the two elements shows a sign different from the target sign
                    target_sign=-1*target_sign # Update the target sign
                    count=0 # Update the count variable that will starts again from zero
                    pass
                
        time_array = time_array+1*np.sign(time_array)
        time_array = np.append(time_array, (len(slice_array)-np.sum(np.abs(time_array)))*target_sign)
                
        time_array[0] = time_array[0] + np.sign(time_array[0])*n_zero
        
        
                
            # print('True')
            # x1 = np.array(np.where(slice_array*slice_array[0]==-1)) # indices where pixel nature is opposite to that of the first element
            ...
            # x2 = np.array(np.where(slice_array[1:]*slice_array[0]==1))+1 # Indices where pixel nature is the same to that of the first element
            ...
            # if time_array.shape!=(0,) and slice_array[time_array[0]]*slice_array[0]==1:
            ...
            # # time_array = np.append(np.array([n_zero]),time_array)im_x
