...
'''
Stability length calculations:
    stab_len_mode = 1: mean' +- alpha*stdev'
    stab_len_mode = 2: mean' +- alpha*mean'
    stab_len_mode = 3: stdev' +- alpha*stdev'
    alpha1, alpha2, alpha3 are the scaling factors, respectively
    (where ' means the last value mean or stdev)
'''
stab_len_mode = 3
...
for i in range(2,len(report_matrix[0,:,0])):
    ...
    if stab_len_mode == 1:  
        plt.axhline(y=mean_matrix[i,-1]+alpha1*std_matrix[i,-1], color='forestgreen', linestyle='--') # Draw horizontal line
        plt.axhline(y=mean_matrix[i,-1]-alpha1*std_matrix[i,-1], color='forestgreen', linestyle='--') # Draw horizontal line
        plt.fill_between(np.array([-1, np.max(mean_matrix[1,:]*px_x)])+1, mean_matrix[i,-1]-alpha1*std_matrix[i,-1], mean_matrix[i,-1]+alpha1*std_matrix[i,-1], color='#9fdae3', alpha=0.75)
    elif stab_len_mode == 2:
    ...
    # plt.axvline(x=6, color='dimgrey', linestyle='--') # Draw vertical line
    axs.set_title(run, fontsize=14)
    axs.set_xlim(0, np.max(mean_matrix[1,:]*px_x)+1)
    axs.set_xlabel('Window analysis length [m]', fontsize=12)
    axs.set_ylabel(label, fontsize=12)
    ax2.set_ylabel('STDEV', color='red', fontsize=12)
    # plt.savefig(os.path.join(plot_dir, run +'_morphW_interp.png'), dpi=200)
    plt.show()
