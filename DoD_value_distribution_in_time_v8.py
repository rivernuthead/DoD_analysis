...
for run in runs:
    ...
    for timespan in range(0,data_raw.shape[3]//2):
        jump = timespan + 1
        for t in range(0,data_raw.shape[0]-1-jump):
            print(timespan, t)
            ...
            if timespan ==0:
                ...
                data = data[:-t,:,:,timespan]
                print('Time: [:' + str(-t) + '],  Timespan: ' , timespan)
    ...
    # merge_pdfs(pdf_overall_dist_path, os.path.join(output_dir, 'REPORT_' + run + '_overall_distribution_SCO_FILL.pdf'))
